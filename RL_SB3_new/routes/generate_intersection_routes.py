    ...
    for traffic_light in traffic_lights_list:
        if traffic_light.id not in traffic_light_visited:
            ...
            if not args.subsample:
                root, route_id = add_intersection(
                    traffic_light.get_transform(), root, route_id
                )
            else:
                root, route_id = add_intersection_subsample(
                    traffic_light.get_transform(), root, route_id
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--town", type=str, default="Town10HD_Opt", help="town for generating routes"
    )
    parser.add_argument(
        "--subsample",
        action="store_true",
        default=False,
        help="sample 75% fewer routes",
    )
